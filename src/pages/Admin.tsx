...
import {
  ...
  MessageSquare, BarChart3, Plus, Trash2, Link, Upload, CheckCircle2,
  FileSpreadsheet, Database, ChevronRight, Search, ShieldAlert, Cpu
} from "lucide-react";
...
const Admin = () => {
  ...
  return (
    <Layout showNav={false}>
      <section className="px-4 max-w-4xl mx-auto space-y-6 pb-12">
        <header className="flex flex-col sm:flex-row items-center justify-between gap-4 glass p-5 rounded-2xl border-white/5 shadow-xl">
          ...
          <div className="flex items-center gap-2">
            <GlassButton variant="outline" onClick={fetchData} disabled={refreshing} className="p-2">
              <RefreshCw className={`w-4 h-4 ${refreshing ? "animate-spin" : ""}`} />
            </GlassButton>
            <GlassButton variant="outline" onClick={() => navigate("/dashboard")} className="px-3 py-2 text-xs">Home</GlassButton>
            <GlassButton variant="outline" onClick={signOut} className="p-2 hover:text-destructive"><LogOut className="w-4 h-4" /></GlassButton>
          </div>
        </header>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <GlassCard animate={false} variant={isActive ? "glow" : "default"} className="text-center p-3 cursor-pointer min-w-[70px]">
                  <Icon className="w-4 h-4 text-primary mx-auto mb-1" />
                  <p className="font-display text-lg font-bold text-foreground">{tab.count || '0'}</p>
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto">
                    {verificationTxs.map(tx => (
                      <div key={tx.id} className="flex items-center justify-between glass rounded-xl p-3">
                        <div>
                          <p className="text-xs text-muted-foreground">{profiles.find(p => p.id === tx.user_id)?.email || 'User'}</p>
                        ...
                        <div className="text-right">
                          {tx.is_verified ? (
                            <div className="flex items-center gap-1 text-primary">
                              ...
                              <span className="text-xs">₦{tx.verified_amount?.toLocaleString()}</span>
                            </div>
                          ) : (
