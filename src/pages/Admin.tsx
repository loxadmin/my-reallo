...
const Btn = ({ children, variant = "default", className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "default" | "primary" | "outline" | "destructive" }) => {
  const base = "inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all duration-300 disabled:opacity-50 disabled:pointer-events-none cursor-pointer";
          ...
          <header className="sticky top-0 z-40 h-14 flex items-center justify-between border-b border-border/40 bg-background/70 backdrop-blur-xl px-6">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="rounded-lg border border-border/40 bg-muted/30 text-sm text-foreground px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary/30 cursor-pointer"
              >
              ...
              <>
                ...
                <TableCard className="overflow-hidden">
                  <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-5 text-sm mr-4">
                        {[
                ...
                <TableCard>
                  ...
                  {activities.slice(0, 6).map((a) => (
                    <TableRow key={a.id}>
                      <span className="flex-1 min-w-0 text-sm text-muted-foreground font-mono truncate">{a.user_id.slice(0, 8)}</span>
                      <span className="w-24 shrink-0 text-sm text-foreground capitalize">{a.action_type}</span>
                      <span className="w-20 shrink-0 text-right text-sm font-semibold text-primary">+{a.positions_moved}</span>
                      ...
                      <div key={p.id}>
                        <TableRow onClick={() => { setSelectedUserId(isSelected ? null : p.id); setEditingProfile(null); }}>
                          <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            <p className="text-sm text-muted-foreground mt-0.5">Joined {new Date(p.created_at).toLocaleDateString()}</p>
                          </div>
                          ...
                          <span className="w-16 shrink-0">
                            {p.is_banned ? <StatusBadge status="BANNED" /> : pDuplicates.length > 0 ? <span className="text-sm bg-destructive/10 text-destructive px-1.5 py-0.5 rounded-full border border-destructive/20">{pDuplicates.length} dup</span> : <StatusBadge status="active" />}
                          </span>
                ...
                <div className="max-h-[600px] overflow-y-auto">
                  {activities.map((a) => (
                    <TableRow key={a.id}>
                      ...
                      <span className="w-28 text-right text-sm text-muted-foreground">{new Date(a.created_at).toLocaleDateString()}</span>
                    </TableRow>
                  ))}
                  {activities.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No activity yet</div>}
                </div>
                      ...
                      <div className="px-5 py-4 border-b border-border/30 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          {app.app_logo_url ? (
                            ...
                            <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-sm font-bold text-primary">{app.app_name.charAt(0)}</div>
                          )}
                          <div>
                      ...
                      return (
                        <TableRow key={tx.id} className={tx.is_duplicate ? "bg-destructive/5" : ""}>
                          ...
                          <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            {tx.is_duplicate && <p className="text-sm text-destructive truncate">{tx.duplicate_note || 'Duplicate'}</p>}
                          </div>
                    ...
                    {userWarnings.map(w => {
                      ...
                      return (
                        <TableRow key={w.id}>
                          ...
                          <span className="flex-[2] text-sm text-muted-foreground">{w.reason}</span>
                ...
                <TableCard>
                  ...
                  <div className="max-h-[500px] overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {infReferrals.map((r: any) => {
                      return (
                        <TableRow key={r.id}>
                          <span className="flex-1 min-w-0 flex items-center gap-1 text-sm"><button onClick={() => setDrawerUserId(r.influencer_id)} className="text-primary hover:underline truncate">{profiles.find(p => p.id === r.influencer_id)?.email || r.influencer_id?.slice(0, 8)}</button> → <button onClick={() => setDrawerUserId(r.referred_user_id)} className="text-primary hover:underline truncate">{profiles.find(p => p.id === r.referred_user_id)?.email || r.referred_user_id?.slice(0, 8)}</button></span>
                          <span className="w-20 shrink-0 text-right text-sm text-primary font-semibold">{formatNaira(r.reward_amount)}</span>
              ...
              <div className="space-y-6">
                <div className={cardCls}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-xs text-muted-foreground mb-1">Challenge Type</p>
                      ...
                      <div><p className="text-xs text-muted-foreground mb-1">Reward per video (₦)</p><input type="number" value={newChallenge.reward_per_video} onChange={e => setNewChallenge(p => ({ ...p, reward_per_video: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                    </div>
                    {newChallenge.challenge_type === "set" && (
                      <div className="grid grid-cols-2 gap-3">
                        <div><p className="text-xs text-muted-foreground mb-1">Total Videos</p><input type="number" value={newChallenge.total_videos} onChange={e => setNewChallenge(p => ({ ...p, total_videos: parseInt(e.target.value) || 1 }))} min={2} className={inputCls} /></div>
                        <div><p className="text-xs text-muted-foreground mb-1">Post interval (days)</p><input type="number" value={newChallenge.posting_interval_days} onChange={e => setNewChallenge(p => ({ ...p, posting_interval_days: parseInt(e.target.value) || 1 }))} min={1} className={inputCls} /></div>
                      </div>
                    )}
                    <Btn variant="primary" onClick={async () => {
                              ...
                              <div key={sub.id} className="rounded-lg border border-border/40 p-3">
                                <div className="flex items-center justify-between mb-2">
                                  <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">Video #{sub.video_number} of {ch.total_videos}</p>
                                  </div>
                        ...
                        {pending.map((sub: any) => {
                          ...
                          return (
                            <TableRow key={sub.id}>
                              ...
                              <span className="flex-1 text-sm text-muted-foreground truncate">{challenge?.title || "Unknown"}</span>
                              <span className="w-20 text-center text-sm text-foreground">#{sub.video_number}{challenge ? ` / ${challenge.total_videos}` : ""}</span>
                    ...
                    {[...infChallengeSubmissions].sort((a: any, b: any) => new Date(b.submitted_at).getTime() - new Date(a.submitted_at).getTime()).map((sub: any) => {
                      ...
                      return (
                        <TableRow key={sub.id}>
                          <span className="flex-1 truncate"><UserLink userId={sub.user_id} /></span>
                          <span className="flex-1 text-sm text-muted-foreground truncate">{challenge?.title || "Unknown"}</span>
                          <span className="w-20 text-center text-sm">#{sub.video_number}</span>
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between rounded-lg border border-border/40 p-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">If disabled, users will see "Coming Soon".</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="text-sm text-muted-foreground font-medium">Verify Expense Button Link</label><input value={verifyExpenseLink} onChange={e => setVerifyExpenseLink(e.target.value)} placeholder="https://..." className={`${inputCls} mt-1.5`} /></div>
                    <div><label className="text-sm text-muted-foreground font-medium">Post-Queue Referral Points</label><input type="number" value={postQueueReferralPoints} onChange={e => setPostQueueReferralPoints(e.target.value)} className={`${inputCls} mt-1.5`} /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="text-sm text-muted-foreground font-medium">Verify Spend Link</label><input value={verifySpendLink} onChange={e => setVerifySpendLink(e.target.value)} placeholder="https://..." className={`${inputCls} mt-1.5`} /></div>
                    <div><label className="text-sm text-muted-foreground font-medium">Verify Spend Description</label><textarea value={verifySpendDescription} onChange={e => setVerifySpendDescription(e.target.value)} className={`${inputCls} mt-1.5 min-h-[60px] resize-none`} /></div>
                  </div>
                  <hr className="border-border/30" />
                  <h4 className="text-sm font-semibold text-foreground">Footer Content</h4>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div><label className="text-sm text-muted-foreground font-medium">About Us</label><textarea value={footerAboutUs} onChange={e => setFooterAboutUs(e.target.value)} className={`${inputCls} mt-1.5 min-h-[80px] resize-none`} /></div>
                    <div><label className="text-sm text-muted-foreground font-medium">Contact Us</label><textarea value={footerContactUs} onChange={e => setFooterContactUs(e.target.value)} className={`${inputCls} mt-1.5 min-h-[80px] resize-none`} /></div>
                    <div><label className="text-sm text-muted-foreground font-medium">Invest With Us</label><textarea value={footerInvestWithUs} onChange={e => setFooterInvestWithUs(e.target.value)} className={`${inputCls} mt-1.5 min-h-[80px] resize-none`} /></div>
                  </div>
                  <hr className="border-border/30" />
                  <h4 className="text-sm font-semibold text-foreground">Currency Exchange Rates</h4>
                  <p className="text-sm text-muted-foreground -mt-3">Set how many Naira (₦) equals 1 unit of each currency. Users see amounts in their local currency based on geolocation.</p>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="text-sm text-muted-foreground font-medium">1 USD = ₦</label>
                    ...
                    <div>
                      <label className="text-sm text-muted-foreground font-medium">1 EUR = ₦</label>
                    ...
                    <div>
                      <label className="text-sm text-muted-foreground font-medium">1 GBP = ₦</label>
