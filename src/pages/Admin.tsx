    ...
    <Sidebar collapsible="icon" className="border-r-0">
      <SidebarHeader className={`${collapsed ? "px-2 py-4" : "px-5 py-6"} border-b border-sidebar-border/40 transition-all`}>
        <div className="flex items-center justify-center gap-3">
          ...
          {!collapsed && <span className="text-sm font-bold text-sidebar-foreground tracking-tight">Reallo Admin</span>}
        </div>
      </SidebarHeader>
      <SidebarContent className={`${collapsed ? "px-1" : "px-4"} py-4 overflow-y-auto scrollbar-hide transition-all`} style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {navGroups.map((group) => (
          <SidebarGroup key={group.label} className="mb-3">
            <SidebarGroupLabel className="text-xs uppercase tracking-[0.18em] text-sidebar-foreground/40 font-semibold mb-2 px-3">
              {group.label}
            </SidebarGroupLabel>
            <SidebarGroupContent>
                        ...
                        {!collapsed && (
                          <div className="flex items-center justify-between w-full min-w-0">
                            ...
                            {count !== undefined && count > 0 && (
                              <span className={`text-xs min-w-[22px] text-center px-1.5 py-0.5 rounded-md font-bold shrink-0 ml-2 ${isActive ? "bg-primary-foreground/25 text-primary-foreground" : "bg-primary/15 text-primary"}`}>
                                {count > 99 ? "99+" : count}
                              </span>
                            )}
                          </div>
  ...
  <div className="rounded-xl border border-border/50 bg-card/80 backdrop-blur-sm p-5 space-y-2.5 hover:shadow-md transition-shadow overflow-hidden">
    ...
    {trend && trendLabel && (
      <div className="flex items-center gap-1.5">
        ...
        <span className={`text-xs font-medium ${trend === "up" ? "text-primary" : trend === "down" ? "text-destructive" : "text-muted-foreground"}`}>{trendLabel}</span>
      </div>
    )}
  </div>
);
...
const StatusBadge = ({ status, className }: { status: string; className?: string }) => {
  ...
  return <span className={`text-xs px-2.5 py-1 rounded-full font-medium border ${colors[status] || "bg-muted text-muted-foreground border-border"} ${className || ""}`}>{status.replace(/_/g, " ")}</span>;
};
...
const Btn = ({ children, variant = "default", className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "default" | "primary" | "outline" | "destructive" }) => {
  const base = "inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-semibold transition-all duration-300 disabled:opacity-50 disabled:pointer-events-none cursor-pointer";
...
const TableHeader = ({ children }: { children: React.ReactNode }) => (
  <div className="px-5 py-3 border-b border-border/30 bg-muted/20">
    <div className="flex items-center text-xs font-semibold text-muted-foreground uppercase tracking-wider gap-3">
      {children}
    </div>
  </div>
);
...
const Admin = () => {
  ...
  const inputCls = "w-full rounded-lg border border-border/60 bg-background/50 backdrop-blur-sm px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all";
  ...
  const downloadFinancialStatement = (format: "csv" | "pdf") => {
    ...
    const lines = [
      ...
      ["Note: Platform profit is 40% of processed revenue"],
      ["", `Amount (${sym})`],
      ["── REVENUE ──", ""],
      ["Processed Revenue (verified transactions)", `${sym}${fmtVal(totalVerified)}`],
      ["Platform Revenue (40% of processed)", `${sym}${fmtVal(totalVerified * 0.4)}`],
      ["", ""],
      ["── COSTS & LIABILITIES ──", ""],
      ...
      ["", ""],
      ["── NET POSITION ──", ""],
      ["Gross Profit (40% of processed revenue)", `${sym}${fmtVal(totalVerified * 0.4)}`],
      ["Net Profit (Gross Profit - All Costs)", `${sym}${fmtVal(totalVerified * 0.4 - (totalInfluencerPayouts + pendingInfluencerPayouts + totalInfluencerBalance + totalPoints * 0.5))}`],
      ["Net Profit Margin", totalVerified > 0 ? `${((totalVerified * 0.4 - (totalInfluencerPayouts + pendingInfluencerPayouts + totalInfluencerBalance + totalPoints * 0.5)) / (totalVerified * 0.4) * 100).toFixed(1)}%` : "N/A"],
      [],
      ["═══ 4. POINTS ECONOMY ═══"],
      ["Metric", "Value"],
  ...
  const UserLink = ({ userId }: { userId: string }) => {
    ...
    return (
      <button onClick={() => setDrawerUserId(userId)} className="text-sm font-semibold text-primary hover:underline cursor-pointer text-left truncate">
        {email}
      </button>
    );
  };
          ...
          <header className="sticky top-0 z-40 h-14 flex items-center justify-between border-b border-border/40 bg-background/70 backdrop-blur-xl px-6">
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-4 py-1.5 rounded-lg border border-border/40 bg-muted/30 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-primary/30 w-[240px]"
                />
              </div>
            </div>
            <div className="flex items-center gap-2">
              <select
                ...
                className="rounded-lg border border-border/40 bg-muted/30 text-xs text-foreground px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary/30 cursor-pointer"
              >
              ...
              <>
                ...
                <TableCard className="overflow-hidden">
                  <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3">
                    <div>
                      <h3 className="text-sm font-bold text-foreground">Platform Activity</h3>
                ...
                <TableCard>
                  ...
                  {activities.slice(0, 6).map((a) => (
                    <TableRow key={a.id}>
                      <span className="flex-1 min-w-0 text-xs text-muted-foreground font-mono truncate">{a.user_id.slice(0, 8)}</span>
                      <span className="w-24 shrink-0 text-xs text-foreground capitalize">{a.action_type}</span>
                      <span className="w-20 shrink-0 text-right text-xs font-semibold text-primary">+{a.positions_moved}</span>
                      <span className="w-24 shrink-0 text-right text-xs text-muted-foreground">{new Date(a.created_at).toLocaleDateString()}</span>
                    </TableRow>
                  ))}
                  {activities.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No activity yet</div>}
                </TableCard>
              </>
            )}
                      ...
                      <div key={p.id}>
                        <TableRow onClick={() => { setSelectedUserId(isSelected ? null : p.id); setEditingProfile(null); }}>
                          <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">Joined {new Date(p.created_at).toLocaleDateString()}</p>
                          </div>
                          <span className="w-20 shrink-0 text-sm text-foreground">{formatNairaCompact(p.total_annual_spend || 0)}</span>
                          <span className="w-14 shrink-0 text-sm text-muted-foreground">#{p.queue_position}</span>
                          <span className="w-16 shrink-0 text-sm text-foreground">{formatCompact(p.points_balance)}</span>
                          <span className="w-10 shrink-0 text-sm text-muted-foreground">{referralCounts[p.id] || 0}</span>
                          <span className="w-16 shrink-0">
                            {p.is_banned ? <StatusBadge status="BANNED" /> : pDuplicates.length > 0 ? <span className="text-xs bg-destructive/10 text-destructive px-1.5 py-0.5 rounded-full border border-destructive/20">{pDuplicates.length} dup</span> : <StatusBadge status="active" />}
                          </span>
                          ...
                          <div className="px-5 py-4 bg-muted/10 border-b border-border/20 space-y-3 overflow-hidden">
                            ...
                            {pDuplicates.length > 0 && (
                              <div className="rounded-lg bg-destructive/5 border border-destructive/20 p-3">
                                <p className="text-xs text-destructive font-semibold mb-1">Duplicate Transactions:</p>
                                {pDuplicates.map(d => (
                                  <p key={d.id} className="text-xs text-muted-foreground font-mono">{d.transaction_id} — {d.duplicate_note}</p>
                                ))}
                              </div>
                            )}
                            ...
                            {pWarnings.length > 0 && (
                              <div className="rounded-lg bg-destructive/5 border border-destructive/20 p-3">
                                <p className="text-xs font-semibold text-destructive mb-1">Warning History:</p>
                                {pWarnings.map(w => (
                                  <p key={w.id} className="text-xs text-muted-foreground">{new Date(w.created_at).toLocaleDateString()} — {w.reason}</p>
                                ))}
                              </div>
                            )}
                          </div>
            ...
            {activeTab === "activity" && (
              <TableCard>
                ...
                <div className="max-h-[600px] overflow-y-auto">
                  {activities.map((a) => (
                    <TableRow key={a.id}>
                      <span className="flex-1 text-sm text-muted-foreground font-mono">{a.user_id.slice(0, 12)}...</span>
                      <span className="w-32 text-sm text-foreground capitalize">{a.action_type}</span>
                      <span className="w-24 text-right text-sm font-semibold text-primary">+{a.positions_moved}</span>
                      <span className="w-28 text-right text-xs text-muted-foreground">{new Date(a.created_at).toLocaleDateString()}</span>
                    </TableRow>
                  ))}
                  {activities.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No activity yet</div>}
                </div>
                ...
                <div className={cardCls}>
                  ...
                  <div className="space-y-3">
                    ...
                    {(newApp.category === "yes_no" || newApp.category === "robust") && (
                      <>
                        <div className="grid grid-cols-3 gap-3">
                          <div><p className="text-xs text-muted-foreground mb-1">Select pts</p><input type="number" value={newApp.points_select} onChange={e => setNewApp(p => ({ ...p, points_select: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                          <div><p className="text-xs text-muted-foreground mb-1">Switch intent pts</p><input type="number" value={newApp.points_switch_intent} onChange={e => setNewApp(p => ({ ...p, points_switch_intent: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                          <div><p className="text-xs text-muted-foreground mb-1">Switch complete pts</p><input type="number" value={newApp.points_switch_complete} onChange={e => setNewApp(p => ({ ...p, points_switch_complete: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                        </div>
                    ...
                    {newApp.category === "robust" && (
                      <div>
                        <p className="text-xs text-muted-foreground mb-1">Link to Referral Apps (switch options):</p>
                        {referralApps.length === 0 && <p className="text-xs text-muted-foreground">Create referral apps first.</p>}
                        <div className="space-y-1 max-h-[150px] overflow-y-auto">
                          {referralApps.map(ra => (
                            <label key={ra.id} className="flex items-center gap-2 rounded-lg border border-border/40 p-2 cursor-pointer hover:bg-muted/30">
                              ...
                              <span className="text-sm text-foreground">{ra.app_name}</span>
                            </label>
                          ))}
                        </div>
                      </div>
                    ...
                    {newApp.category === "referral" && (
                      <>
                        ...
                        <div><p className="text-xs text-muted-foreground mb-1">Referral points</p><input type="number" value={newApp.referral_points} onChange={e => setNewApp(p => ({ ...p, referral_points: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                      </>
                    )}
                  ...
                  return (
                    <TableCard key={app.id}>
                      <div className="px-5 py-4 border-b border-border/30 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          {app.app_logo_url ? (
                            ...
                            <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-xs font-bold text-primary">{app.app_name.charAt(0)}</div>
                          )}
                          <div>
                            <h4 className="font-semibold text-foreground text-sm">{app.app_name}</h4>
                            <p className="text-xs text-muted-foreground capitalize">{app.category === "yes_no" ? "Yes/No" : app.category === "referral" ? "Referral" : "Robust"} · {appResponses.length} responses</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                      ...
                      {app.category === "robust" && (app.switch_to_referral_app_ids || []).length > 0 && (
                        <div className="px-5 py-2 border-b border-border/20 text-xs text-primary">Linked: {(app.switch_to_referral_app_ids || []).map(id => decisionApps.find(a => a.id === id)?.app_name).filter(Boolean).join(", ")}</div>
                      )}
                      {pendingApprovals.length > 0 && (
                        <div className="p-4 space-y-2">
                          <p className="text-sm text-primary font-semibold mb-2">Pending Approvals ({pendingApprovals.length})</p>
                              ...
                              <div key={pr.id} className="flex items-center justify-between rounded-lg border border-border/40 p-3">
                                <div>
                                  ...
                                  {screenshotUrl && pr.referral_screenshot_url !== "pending_review" && (
                                    <a href={screenshotUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary flex items-center gap-1 hover:underline mt-0.5"><ExternalLink className="w-2.5 h-2.5" /> Screenshot</a>
                                  )}
                                </div>
                    ...
                    <TableCard key={app.id}>
                      <div className="px-5 py-4 border-b border-border/30 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="font-semibold text-foreground text-sm">{app.app_name}</h4>
                            <p className="text-xs text-muted-foreground capitalize">{app.category === "yes_no" ? "Yes/No" : app.category === "referral" ? "Referral" : "Robust"} · {appResponses.length} responses · {pending.length} pending · {approved.length} approved</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {pending.length > 0 && (
                        <div className="p-4 space-y-2 border-b border-border/20">
                          <p className="text-sm text-primary font-semibold mb-2">⏳ Pending Approvals ({pending.length})</p>
                          {pending.map(pr => {
                            const screenshotUrl = getPublicUrl(pr.referral_screenshot_url);
                            return (
                              <div key={pr.id} className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 p-3">
                                <div className="flex-1 min-w-0">
                                  ...
                                  {screenshotUrl && pr.referral_screenshot_url !== "pending_review" && (
                                    <a href={screenshotUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary flex items-center gap-1 hover:underline mt-1"><ExternalLink className="w-2.5 h-2.5" /> View Screenshot</a>
                                  )}
                                </div>
                                <Btn variant="primary" onClick={() => handleApproveReferral(pr.id, pr.app_id, pr.user_id)}><Check className="w-3 h-3" /> Approve</Btn>
                              </div>
                      ...
                      {/* All responses */}
                      <div className="p-4">
                        <p className="text-sm text-muted-foreground font-semibold mb-2">All Responses ({appResponses.length})</p>
                    ...
                    {verificationTxs.map(tx => {
                      return (
                        <TableRow key={tx.id} className={tx.is_duplicate ? "bg-destructive/5" : ""}>
                          ...
                          <div className="flex-1 min-w-0 overflow-hidden">
                            <p className="text-sm font-mono text-foreground truncate">{tx.transaction_id}</p>
                            {tx.is_duplicate && <p className="text-xs text-destructive truncate">{tx.duplicate_note || 'Duplicate'}</p>}
                          </div>
                          <span className="w-24 shrink-0 flex justify-end">
                            {tx.is_duplicate ? (
                              <span className="flex items-center gap-1 text-destructive text-xs"><AlertTriangle className="w-3 h-3 shrink-0" /> Dup</span>
                            ) : tx.is_verified ? (
                              <span className="flex items-center gap-1 text-primary text-xs"><CheckCircle2 className="w-3 h-3 shrink-0" /> ₦{tx.verified_amount?.toLocaleString("en-NG")}</span>
                            ) : <span className="text-xs text-muted-foreground">Pending</span>}
                          </span>
                        </TableRow>
                      );
                    })}
                    {verificationTxs.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No transactions submitted yet</div>}
                  </div>
                </TableCard>
              </div>
            )}
                      ...
                      {dupUserIds.map(uid => {
                        ...
                        return (
                          <div key={uid} className="px-5 py-4 border-b border-border/20 last:border-0">
                            <div className="flex items-center justify-between mb-2">
                              <div>
                                <p className="text-sm font-semibold text-foreground">{prof?.email || uid.slice(0, 8)}</p>
                                <p className="text-xs text-muted-foreground">Queue: #{prof?.queue_position} • Points: {prof?.points_balance} • Spend: {formatNaira(prof?.total_annual_spend || 0)}</p>
                              </div>
                              <div className="flex items-center gap-2">
                                {prof?.is_banned && <StatusBadge status="BANNED" />}
                                <span className="text-xs bg-destructive/10 text-destructive px-2.5 py-1 rounded-full border border-destructive/20 font-medium">{userDups.length} dup{userDups.length > 1 ? "s" : ""}</span>
                              </div>
                            </div>
                            <div className="space-y-1 mb-3">
                              {userDups.map(dt => (
                                <div key={dt.id} className="flex items-center justify-between text-xs bg-muted/30 rounded-lg px-3 py-2">
                ...
                <TableCard>
                  ...
                  <div className="max-h-[400px] overflow-y-auto">
                    {userWarnings.map(w => {
                      ...
                      return (
                        <TableRow key={w.id}>
                          <span className="flex-1 text-sm font-medium text-foreground">{userEmail}</span>
                          <span className="flex-[2] text-xs text-muted-foreground">{w.reason}</span>
                          <span className="w-28 text-right text-xs text-muted-foreground">{new Date(w.created_at).toLocaleDateString()}</span>
                        </TableRow>
                      );
                    })}
                    {userWarnings.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No warnings issued yet</div>}
                  </div>
                </TableCard>
              </div>
            )}
            ...
            {activeTab === "inf_apps" && (
              <div className="space-y-6">
                ...
                <TableCard>
                      ...
                      <div key={app.id} className="px-5 py-4 border-b border-border/20 last:border-0">
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            ...
                            <a href={app.social_link} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1"><ExternalLink className="w-2.5 h-2.5" /> {app.social_link}</a>
                          </div>
                          <StatusBadge status={app.status} />
                        </div>
                        {(app.status === "pending_review" || app.status === "pending_appeal") && (
                          <div className="flex gap-2 mt-2">
                            {app.status === "pending_appeal" && <p className="text-xs text-primary w-full mb-1">⚡ Appeal</p>}
                  ...
                  {infApps.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No applications yet</div>}
                </TableCard>
              </div>
            )}
            ...
            {activeTab === "inf_wallets" && (
              <TableCard>
                ...
                {infWallets.map((w: any) => {
                    ...
                    <div key={w.id} className="px-5 py-4 border-b border-border/20 last:border-0">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          {bank && <p className="text-xs text-muted-foreground">{bank.bank_name} · {bank.account_number} · {bank.account_name}</p>}
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-foreground">{formatNaira(w.balance || 0)}</span>
                      ...
                      {bank?.id_document_url && (
                        ...
                        }} className="text-primary hover:underline text-xs flex items-center gap-1 mb-2"><ExternalLink className="w-2.5 h-2.5" /> View ID Document</button>
                      )}
                  ...
                  {infWallets.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No wallet activations yet</div>}
              </TableCard>
            )}
                ...
                <TableCard>
                  ...
                  <div className="max-h-[500px] overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {infReferrals.map((r: any) => {
                      return (
                        <TableRow key={r.id}>
                          <span className="flex-1 min-w-0 flex items-center gap-1 text-xs"><button onClick={() => setDrawerUserId(r.influencer_id)} className="text-primary hover:underline truncate">{profiles.find(p => p.id === r.influencer_id)?.email || r.influencer_id?.slice(0, 8)}</button> → <button onClick={() => setDrawerUserId(r.referred_user_id)} className="text-primary hover:underline truncate">{profiles.find(p => p.id === r.referred_user_id)?.email || r.referred_user_id?.slice(0, 8)}</button></span>
                          <span className="w-20 shrink-0 text-right text-xs text-primary font-semibold">{formatNaira(r.reward_amount)}</span>
                          <span className="w-24 shrink-0 text-right text-xs text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</span>
                        </TableRow>
                      );
                    })}
                    {infReferrals.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No influencer referrals yet</div>}
                  </div>
                </TableCard>
              </div>
            )}
            ...
            {activeTab === "inf_withdrawals" && (
              <TableCard>
                ...
                {infWithdrawals.map((w: any) => {
                  ...
                  return (
                    <div key={w.id} className="px-5 py-4 border-b border-border/20 last:border-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-3">
                          <p className="text-sm font-bold text-primary">{formatNaira(w.amount)}</p>
                          <StatusBadge status={w.status} />
                        </div>
                      </div>
                      {w.status === "pending" && (
                  ...
                  {infWithdrawals.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No withdrawals yet</div>}
              </TableCard>
            )}
              ...
              <div className="space-y-6">
                <div className={cardCls}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-xs text-muted-foreground mb-1 font-medium">Challenge Type</p>
                      ...
                      <div><p className="text-xs text-muted-foreground mb-1 font-medium">Reward per video (₦)</p><input type="number" value={newChallenge.reward_per_video} onChange={e => setNewChallenge(p => ({ ...p, reward_per_video: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                    </div>
                    {newChallenge.challenge_type === "set" && (
                      <div className="grid grid-cols-2 gap-3">
                        <div><p className="text-xs text-muted-foreground mb-1 font-medium">Total Videos</p><input type="number" value={newChallenge.total_videos} onChange={e => setNewChallenge(p => ({ ...p, total_videos: parseInt(e.target.value) || 1 }))} min={2} className={inputCls} /></div>
                        <div><p className="text-xs text-muted-foreground mb-1 font-medium">Post interval (days)</p><input type="number" value={newChallenge.posting_interval_days} onChange={e => setNewChallenge(p => ({ ...p, posting_interval_days: parseInt(e.target.value) || 1 }))} min={1} className={inputCls} /></div>
                      </div>
                    )}
                    <Btn variant="primary" onClick={async () => {
                    ...
                    <TableCard key={ch.id}>
                      ...
                      {pendingSubs.length > 0 && (
                        <div className="p-4 border-t border-border/20 space-y-2">
                          <p className="text-sm text-primary font-semibold">Pending Approvals</p>
                          {pendingSubs.map((sub: any) => {
                            ...
                            return (
                              <div key={sub.id} className="rounded-lg border border-border/40 p-3">
                                <div className="flex items-center justify-between mb-2">
                                  <div>
                                    <p className="text-sm text-foreground font-semibold">{userEmail}</p>
                                    <p className="text-xs text-muted-foreground">Video #{sub.video_number} of {ch.total_videos}</p>
                                  </div>
                                  <a href={sub.video_url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1"><ExternalLink className="w-2.5 h-2.5" /> View</a>
                                </div>
                                <div className="flex gap-2">
                                  <Btn variant="primary" onClick={async () => {
                ...
                {infChallenges.length === 0 && <div className={cardCls}><p className="text-center py-8 text-muted-foreground text-sm">No challenges created yet</p></div>}
              </div>
            )}
            ...
            {activeTab === "inf_submissions" && (
              <div className="space-y-6">
                        ...
                        {pending.map((sub: any) => {
                          ...
                          return (
                            <TableRow key={sub.id}>
                              ...
                              <span className="flex-1 text-xs text-muted-foreground truncate">{challenge?.title || "Unknown"}</span>
                              <span className="w-20 text-center text-xs text-foreground">#{sub.video_number}{challenge ? ` / ${challenge.total_videos}` : ""}</span>
                              <span className="w-20 text-center">
                                {priorRejections > 0
                                  ? <span className="text-xs px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/20 font-medium">Last chance</span>
                                  : <span className="text-xs text-muted-foreground">0</span>
                                }
                              </span>
                              <span className="w-28 text-right text-xs text-muted-foreground">{new Date(sub.submitted_at).toLocaleDateString()}</span>
                              <span className="w-40 flex gap-2 justify-end">
                                <a href={sub.video_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline"><ExternalLink className="w-2.5 h-2.5" />View</a>
                                <Btn variant="primary" onClick={async () => {
                                  await supabase.from("influencer_challenge_submissions" as any).update({ status: "approved", reviewed_at: new Date().toISOString() } as any).eq("id", sub.id);
                ...
                <TableCard>
                  ...
                  <div className="max-h-[500px] overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {infChallengeSubmissions.length === 0 && <div className="py-8 text-center text-muted-foreground text-sm">No submissions yet</div>}
                    {[...infChallengeSubmissions].sort((a: any, b: any) => new Date(b.submitted_at).getTime() - new Date(a.submitted_at).getTime()).map((sub: any) => {
                      ...
                      return (
                        <TableRow key={sub.id}>
                          <span className="flex-1 truncate"><UserLink userId={sub.user_id} /></span>
                          <span className="flex-1 text-xs text-muted-foreground truncate">{challenge?.title || "Unknown"}</span>
                          <span className="w-20 text-center text-xs">#{sub.video_number}</span>
                          <span className="w-24 text-center"><StatusBadge status={sub.status} /></span>
                          <span className="w-28 text-right text-xs text-muted-foreground">{new Date(sub.submitted_at).toLocaleDateString()}</span>
                          <span className="w-16 text-right"><a href={sub.video_url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline"><ExternalLink className="w-3 h-3 inline" /></a></span>
                        </TableRow>
                      );
                    })}
                  </div>
              ...
              <div className={cardCls}>
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between rounded-lg border border-border/40 p-4">
                    <div>
                      <label className="text-sm font-medium text-foreground">Verify Page Active</label>
                      <p className="text-xs text-muted-foreground">If disabled, users will see "Coming Soon".</p>
                    </div>
                  ...
                  <p className="text-sm font-medium text-foreground">Per-Category Verification Toggles</p>
                  <div className="grid grid-cols-2 gap-3">
                    {([
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="text-xs text-muted-foreground font-medium">Verify Expense Button Link</label><input value={verifyExpenseLink} onChange={e => setVerifyExpenseLink(e.target.value)} placeholder="https://..." className={`${inputCls} mt-1.5`} /></div>
                    <div><label className="text-xs text-muted-foreground font-medium">Post-Queue Referral Points</label><input type="number" value={postQueueReferralPoints} onChange={e => setPostQueueReferralPoints(e.target.value)} className={`${inputCls} mt-1.5`} /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="text-xs text-muted-foreground font-medium">Verify Spend Link</label><input value={verifySpendLink} onChange={e => setVerifySpendLink(e.target.value)} placeholder="https://..." className={`${inputCls} mt-1.5`} /></div>
                    <div><label className="text-xs text-muted-foreground font-medium">Verify Spend Description</label><textarea value={verifySpendDescription} onChange={e => setVerifySpendDescription(e.target.value)} className={`${inputCls} mt-1.5 min-h-[60px] resize-none`} /></div>
                  </div>
                  <hr className="border-border/30" />
                  <h4 className="text-sm font-semibold text-foreground">Footer Content</h4>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div><label className="text-xs text-muted-foreground font-medium">About Us</label><textarea value={footerAboutUs} onChange={e => setFooterAboutUs(e.target.value)} className={`${inputCls} mt-1.5 min-h-[80px] resize-none`} /></div>
                    <div><label className="text-xs text-muted-foreground font-medium">Contact Us</label><textarea value={footerContactUs} onChange={e => setFooterContactUs(e.target.value)} className={`${inputCls} mt-1.5 min-h-[80px] resize-none`} /></div>
                    <div><label className="text-xs text-muted-foreground font-medium">Invest With Us</label><textarea value={footerInvestWithUs} onChange={e => setFooterInvestWithUs(e.target.value)} className={`${inputCls} mt-1.5 min-h-[80px] resize-none`} /></div>
                  </div>
                  <hr className="border-border/30" />
                  <h4 className="text-sm font-semibold text-foreground">Currency Exchange Rates</h4>
                  <p className="text-xs text-muted-foreground -mt-3">Set how many Naira (₦) equals 1 unit of each currency. Users see amounts in their local currency based on geolocation.</p>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs text-muted-foreground font-medium">1 USD = ₦</label>
                    ...
                    <div>
                      <label className="text-xs text-muted-foreground font-medium">1 EUR = ₦</label>
                    ...
                    <div>
                      <label className="text-xs text-muted-foreground font-medium">1 GBP = ₦</label>
                  ...
                  <p className="text-xs text-muted-foreground">Geolocation mapping: Nigeria → ₦, USA → $, UK → £, Europe → €, Others → $</p>
              ...
              <div className="space-y-6">
                <div className={cardCls}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div><p className="text-xs text-muted-foreground mb-1">Points Reward</p><input type="number" value={newSurvey.points_reward} onChange={e => setNewSurvey(p => ({ ...p, points_reward: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
                      <div><p className="text-xs text-muted-foreground mb-1">Completion Link</p><input value={newSurvey.completion_link} onChange={e => setNewSurvey(p => ({ ...p, completion_link: e.target.value }))} placeholder="https://..." className={inputCls} /></div>
                    </div>
                    ...
                    <TableCard key={survey.id}>
                      <div className="px-5 py-4 border-b border-border/30 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{survey.points_reward} pts · {sQuestions.length} questions · {sResponses.length} total responses</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Btn variant="outline" onClick={async () => {
                      ...
                      <div className="p-4 bg-muted/5 border-b border-border/10">
                        <p className="text-sm font-semibold mb-2">Add Question</p>
                        <div className="space-y-2">
                          <input
                      ...
                      <div className="p-4 border-b border-border/10">
                        <p className="text-sm text-muted-foreground font-semibold mb-2">Questions ({sQuestions.length})</p>
                        <div className="space-y-2">
                          {sQuestions.map((q, idx) => {
                            const qOpts = surveyOptions.filter(o => o.question_id === q.id);
                            return (
                              <div key={q.id} className="text-sm p-2 rounded-lg border border-border/40">
                            ...
                            {pending.map(pr => {
                              ...
                              return (
                                <div key={pr.id} className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 p-3">
                                  <div className="flex-1 min-w-0">
                                    <UserLink userId={pr.user_id} />
                                    <p className="text-xs text-muted-foreground mt-0.5">{new Date(pr.created_at).toLocaleDateString()}</p>
                                    {screenshotUrl && (
                                      <a href={screenshotUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary flex items-center gap-1 hover:underline mt-1"><ExternalLink className="w-2.5 h-2.5" /> View Proof</a>
                                    )}
                                  </div>
                                  <div className="flex gap-2">
