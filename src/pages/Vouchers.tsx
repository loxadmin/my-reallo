        ...
        <div className="w-full max-w-md space-y-4">
          ...
          {!blockMessage && (
            <GlassCard variant="strong">
              ...
              <p className="text-sm text-muted-foreground mb-3">Minimum 100,000 points (₦50,000). Max claimable: {formatNaira(claimableAmount)}.</p>
              ...
              {Number(pointsToUse) > 0 && (
                <p className="text-sm text-primary font-semibold mb-3">
                  ...
                  {nairaValue > claimableAmount && <span className="text-destructive text-sm ml-2">(exceeds claimable)</span>}
                </p>
              )}
                  ...
                  <div key={v.id} className="glass rounded-xl p-3">
                    <div className="relative overflow-hidden rounded-xl p-4 mb-2" style={{ background: "linear-gradient(135deg, hsl(160 60% 18% / 0.15), hsl(160 50% 25% / 0.1))", border: "1px solid hsl(160 60% 18% / 0.15)" }}>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs text-muted-foreground uppercase tracking-[0.12em]">Reallo Voucher</p>
                      ...
                      <p className="font-mono text-sm text-primary mt-2 tracking-[0.12em]">{v.voucher_code}</p>
                    </div>
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-muted-foreground">{new Date(v.created_at).toLocaleDateString()} • {v.points_used.toLocaleString()} pts</p>
