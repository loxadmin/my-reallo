      ...
      <GlassCard variant="strong" className="space-y-5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">All categories verified</p>
          </div>
        </div>
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass rounded-2xl p-5 text-center border border-primary/20">
          ...
          <p className="font-semibold text-foreground text-sm">Verification Complete!</p>
          <p className="text-sm text-primary font-semibold mt-1">
            {formatCurrency(totalVerifiedAnnualSpend)}
          </p>
          <p className="text-xs text-muted-foreground">Total Verified Annual Spend</p>
              ...
              return (
                <div key={t} className="glass rounded-xl p-2.5 text-center">
                  <div className="flex items-center justify-center gap-1.5 text-muted-foreground mb-1">
                    {spendMeta[t].icon}
                    <span className="text-xs">{spendMeta[t].label}</span>
                  </div>
                  <p className="text-sm font-semibold text-foreground">{formatCurrency(spend)}</p>
                </div>
              );
            })}
          </div>
    ...
    return (
      <GlassCard variant="strong" className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-foreground text-sm">{meta.label} Verification</h3>
        ...
        {complete ? (
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="glass rounded-2xl p-5 text-center border border-primary/20">
            ...
            <p className="font-semibold text-foreground text-sm">{meta.label} Verified!</p>
          ...
          <div className="space-y-4">
            {verifySettings.description && (
              <div className="glass rounded-xl p-4 border border-border">
                <p className="text-sm text-foreground leading-relaxed">{verifySettings.description}</p>
              </div>
            )}
              ...
              <a href={verifySettings.link} target="_blank" rel="noopener noreferrer" className="block">
                <div className="glass rounded-xl p-3.5 flex items-center justify-between hover:border-primary/30 transition-colors border border-border">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm text-foreground font-medium">Open verification link</span>
                  </div>
            ...
            {type === "data" && (
              <div className="space-y-2.5">
                <p className="text-xs text-muted-foreground font-medium">How often do you buy data?</p>
                <div className="grid grid-cols-3 gap-2">
                  {(["daily", "weekly", "monthly"] as const).map(f => (
                    <button
                      key={f}
                      onClick={() => setFrequency(f)}
                      className={`glass rounded-xl py-3 text-sm capitalize font-medium transition-all border ${
                        frequency === f
            ...
            <GlassButton variant="primary" className="w-full text-sm py-3.5" onClick={() => handleStartVerification(type)} disabled={starting}>
              {starting ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2.5 py-0.5 rounded-full border font-bold uppercase tracking-widest ${config.badgeClass}`}>
                    {config.label}
                  </span>
                </div>
    ...
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl p-3.5 space-y-2.5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-medium text-foreground capitalize">{verification.frequency}</span>
        </div>
      ...
      {verifiedCount > 0 && annualSpend > 0 && (
        <div className="glass rounded-xl p-4 border border-primary/15 text-center">
          <p className="text-xs text-muted-foreground mb-1">Estimated Annual {meta.label} Spend</p>
          <p className="text-lg font-bold text-primary">{formatCurrency(annualSpend)}</p>
      ...
      {/* Transaction inputs */}
      <div className="space-y-2">
        <p className="text-xs font-medium text-foreground">Transaction IDs</p>
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <div className="flex-1">
                    {isEditing ? (
                      ...
                      <GlassInput
                        ...
                        className={`text-sm ${isDuplicate ? "border-destructive/30" : isSubmitted && existingTx.is_verified ? "border-primary/30" : ""}`}
                      />
                    )}
                  </div>
                  ...
                  {isEditing ? (
                    <div className="flex gap-1">
                      <GlassButton variant="primary" onClick={() => onEditTx(existingTx)} disabled={submitting || !editValue.trim()} className="px-3 py-2 text-xs">
                        Save
                      </GlassButton>
                      <GlassButton variant="outline" onClick={() => { setEditingTxId(null); setEditValue(""); }} className="px-2 py-2 text-xs">
                        ✕
                      </GlassButton>
                    </div>
                  ) : isSubmitted ? (
                    existingTx.is_verified ? (
                      <div className="flex items-center gap-1 text-primary shrink-0">
                        ...
                        <span className="text-xs font-medium">{formatCurrency(existingTx.verified_amount ?? 0)}</span>
                      </div>
                    ) : isDuplicate ? (
                      canEdit ? (
                        <GlassButton
                          ...
                          className="px-3 py-2 text-xs text-destructive border-destructive/20"
                        >
                          Fix
                        </GlassButton>
                      ) : (
                        <span className="text-xs text-destructive font-medium shrink-0">Duplicate</span>
                      )
                    ) : (
                    ...
                    <GlassButton variant="primary" onClick={() => onSubmitTx(idx)} disabled={submitting || !val.trim()} className="px-3 py-2 text-xs">
                      {submitting ? <Loader2 className="w-3 h-3 animate-spin" /> : "Submit"}
                    </GlassButton>
                  )}
                </div>
                {isDuplicate && !isEditing && canEdit && (
                  <p className="text-xs text-destructive/80 ml-7 flex items-center gap-1 font-medium">
