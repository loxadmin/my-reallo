        ...
        <AnimatePresence mode="wait">
          ...
          {selectedType && hasSubcategories && !selectedCategory && (
            <motion.div key="subcategories" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              <button onClick={() => setSelectedType(null)} className="flex items-center gap-1 text-sm text-muted-foreground mb-4 hover:text-primary transition-colors">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Max: {formatNaira(cat.max_price)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-muted-foreground">You can claim</p>
            ...
            <motion.div key="confirm" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              ...
              <GlassCard variant="glow">
                <div className="text-center">
                  <p className="text-sm text-muted-foreground uppercase tracking-[0.12em] mb-1">{goalMeta[selectedType!]?.label || selectedType}</p>
                  <h3 className="text-lg font-bold text-foreground mb-1">{selectedCategory.label}</h3>
                  <p className="text-sm text-muted-foreground mb-6">Max price set: {formatNaira(selectedCategory.max_price)}</p>
                  ...
                  <p className="text-sm text-muted-foreground">{formatNaira(claimableAmount)} / {formatNaira(selectedCategory.max_price)} ({progress.toFixed(1)}%)</p>
