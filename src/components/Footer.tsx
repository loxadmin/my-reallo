        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {content.about_us && (
            <div>
              ...
              <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-line"><RealloLogo size={12} className="inline-block align-middle mr-0.5" /> {content.about_us}</p>
            </div>
          )}
          {content.contact_us && (
            <div>
              <h4 className="text-sm font-semibold text-foreground mb-2">Contact Us</h4>
              <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-line">{content.contact_us}</p>
            </div>
          )}
          {content.invest_with_us && (
            <div>
              <h4 className="text-sm font-semibold text-foreground mb-2">Invest With Us</h4>
              <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-line">{content.invest_with_us}</p>
            </div>
          )}
        </div>
        <div className="mt-8 pt-4 border-t border-border/20 text-center">
          <p className="text-xs text-muted-foreground font-medium uppercase tracking-tight opacity-70">© {new Date().getFullYear()} Reallo. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
