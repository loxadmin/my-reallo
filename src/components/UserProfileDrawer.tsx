...
const Field = ({ label, value, mono }: { label: string; value: React.ReactNode; mono?: boolean }) => (
  <div className="flex items-center justify-between py-2 border-b border-border/15 last:border-0">
    <span className="text-[13px] text-muted-foreground uppercase tracking-wider font-medium">{label}</span>
    <span className={`text-[13px] text-foreground font-medium ${mono ? "font-mono" : ""}`}>{value || "—"}</span>
  </div>
);
...
const StatusPill = ({ status }: { status: string }) => {
  const colors: Record<string, string> = {
    ...
    pending_activation: "bg-accent/10 text-accent",
    rejected: "bg-destructive/10 text-destructive",
    closed: "bg-destructive/10 text-destructive",
    appeal_rejected: "bg-destructive/10 text-destructive",
  };
  return <span className={`text-[13px] px-2 py-0.5 rounded-full font-medium ${colors[status] || "bg-muted text-muted-foreground"}`}>{status.replace(/_/g, " ")}</span>;
};
          ...
          >
            ...
            <div className="px-6 py-5 border-b border-border/30 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="text-[13px] text-muted-foreground font-mono mt-0.5">{profile.id.slice(0, 16)}…</p>
              </div>
              <div className="flex items-center gap-2">
                {profile.is_banned && <span className="text-[13px] px-2 py-1 bg-destructive/10 text-destructive rounded-full font-medium flex items-center gap-1"><Ban className="w-3 h-3" /> Banned</span>}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5" style={{ scrollbarWidth: "thin" }}>
              ...
              <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><Shield className="w-3.5 h-3.5 text-primary" /> Account Details</p>
              ...
              {infApp && (
                <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                  <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-primary" /> Influencer Application</p>
                  <Field label="Status" value={<StatusPill status={infApp.status} />} />
                  <Field label="Social Link" value={
                    <a href={infApp.social_link} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1 text-[13px]">
              ...
              {infWallet && (
                <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                  <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><Wallet className="w-3.5 h-3.5 text-primary" /> Influencer Wallet</p>
              ...
              {withdrawals && withdrawals.length > 0 && (
                <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                  <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5 text-primary" /> Withdrawals ({withdrawals.length})</p>
                  {withdrawals.slice(0, 5).map((w: any) => (
                    <div key={w.id} className="flex items-center justify-between py-1.5 border-b border-border/10 last:border-0">
                      <span className="text-[13px] font-medium text-foreground">{formatNaira(w.amount)}</span>
                      <div className="flex items-center gap-2">
                        <StatusPill status={w.status} />
                        <span className="text-[13px] text-muted-foreground">{new Date(w.created_at).toLocaleDateString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {challengeSubmissions && challengeSubmissions.length > 0 && (
                <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                  <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-primary" /> Challenge Submissions ({challengeSubmissions.length})</p>
                  {challengeSubmissions.slice(0, 5).map((s: any) => (
                    <div key={s.id} className="flex items-center justify-between py-1.5 border-b border-border/10 last:border-0">
                      <span className="text-[13px] text-foreground">Video #{s.video_number}</span>
                      <div className="flex items-center gap-2">
              ...
              {verificationTxs && verificationTxs.length > 0 && (
                <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                  <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><Hash className="w-3.5 h-3.5 text-primary" /> Verification TXs ({verificationTxs.length})</p>
                  {verificationTxs.slice(0, 5).map((tx: any) => (
                    <div key={tx.id} className="flex items-center justify-between py-1.5 border-b border-border/10 last:border-0">
                      <span className="text-[13px] font-mono text-foreground truncate max-w-[180px]">{tx.transaction_id}</span>
                      <div className="flex items-center gap-2">
                        {tx.is_duplicate ? <span className="text-[13px] text-destructive">Dup</span> :
                          tx.is_verified ? <span className="text-[13px] text-primary">✓ {formatNaira(tx.verified_amount || 0)}</span> :
              ...
              {decisionResponses && decisionResponses.length > 0 && (
                <div className="rounded-xl border border-border/40 bg-muted/10 p-4">
                  <p className="text-[13px] font-semibold text-foreground mb-3 flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-primary" /> Decision Responses ({decisionResponses.length})</p>
                  {decisionResponses.slice(0, 5).map((dr: any) => (
                    <div key={dr.id} className="flex items-center justify-between py-1.5 border-b border-border/10 last:border-0">
                      <span className="text-[13px] text-foreground">{dr.has_app ? "Has app" : "No app"}{dr.would_switch ? " · Would switch" : ""}</span>
                      <span className="text-[13px] text-muted-foreground">{dr.points_awarded}pts</span>
                    </div>
                  ))}
                </div>
              )}
