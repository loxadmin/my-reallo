...
const AdvertiserManagement = ({ onRefresh }: Props) => {
  ...
  const inputCls = "w-full rounded-xl border border-border/40 bg-card/60 px-3 py-2.5 text-foreground text-[13px] placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all";
  const cardCls = "rounded-xl border border-border/50 bg-card/80 backdrop-blur-sm p-5 space-y-4";
  const tabCls = (active: boolean) => `px-4 py-2 rounded-xl text-[12px] font-semibold transition-all cursor-pointer ${active ? "clay-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`;
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button onClick={() => setTab("submissions")} className={tabCls(tab === "submissions")}>
          <span className="flex items-center gap-1.5">
            ...
            {pendingCount > 0 && <span className="bg-destructive text-destructive-foreground text-[12px] px-1.5 py-0.5 rounded-full">{pendingCount}</span>}
          </span>
        </button>
      </div>
      ...
      {tab === "setup" && (
        <div className={cardCls}>
          ...
          <p className="text-[12px] text-muted-foreground -mt-2">Configure founder credentials for the Letter of Intent.</p>

          <div>
            <label className="text-[12px] text-muted-foreground font-medium">Founder Full Name</label>
            <input value={founderName} onChange={e => setFounderName(e.target.value)} placeholder="Full legal name" className={`${inputCls} mt-1`} />
          </div>

          <div>
            <label className="text-[12px] text-muted-foreground font-medium">Founder Signature</label>
            <div className="flex items-center gap-3 mt-1">
              ...
              <label className="flex items-center gap-2 rounded-xl border border-dashed border-border/50 px-4 py-2.5 cursor-pointer hover:border-primary/30 transition-colors">
                ...
                <span className="text-[12px] text-muted-foreground">{founderSigFile ? founderSigFile.name : "Upload signature"}</span>
          ...
          <div className="flex items-center justify-between">
            <div>
              <label className="text-[12px] text-muted-foreground font-medium">Branded Email Required</label>
              <p className="text-[12px] text-muted-foreground">When enabled, advertiser email must match their website domain.</p>
            </div>
            <button
          ...
          <p className="text-[12px] text-muted-foreground -mt-2">When updated, advertisers will see the count on their dashboard.</p>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[12px] text-muted-foreground font-medium">Current User Count</label>
            ...
            <div>
              <label className="text-[12px] text-muted-foreground font-medium">Proceed Link (when 100k reached)</label>
              <input value={advUserCountLink} onChange={e => setAdvUserCountLink(e.target.value)} placeholder="https://..." className={`${inputCls} mt-1`} />
            </div>
          </div>

          <button onClick={handleSaveSettings} disabled={saving} className="clay-primary text-primary-foreground rounded-xl px-5 py-2.5 text-[12px] font-semibold flex items-center gap-1.5 disabled:opacity-50">
        ...
        <div className={cardCls}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground">Generate unique links for advertisers to fill the LOI form.</p>
            </div>
            <button onClick={handleGenerateToken} disabled={generating} className="clay-primary text-primary-foreground rounded-xl px-4 py-2.5 text-[12px] font-semibold flex items-center gap-1.5 disabled:opacity-50">
              <Plus className="w-3.5 h-3.5" /> {generating ? "..." : "Generate Link"}
            </button>
          </div>

          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {tokens.length === 0 && <p className="text-muted-foreground text-[13px] text-center py-4">No links generated yet.</p>}
            {tokens.map((tk: any) => (
              <div key={tk.id} className="flex items-center gap-3 rounded-xl border border-border/30 p-3 hover:bg-muted/10 transition-colors">
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-mono text-foreground truncate">{tk.token.slice(0, 16)}...</p>
                  <p className="text-[12px] text-muted-foreground">{new Date(tk.created_at).toLocaleDateString()} · {tk.status}</p>
                </div>
                <span className={`text-[12px] px-2 py-0.5 rounded-full font-medium ${tk.status === "active" ? "bg-primary/15 text-primary" : tk.status === "used" ? "bg-muted text-muted-foreground" : "bg-destructive/10 text-destructive"}`}>
                  {tk.status}
                </span>
                {tk.status === "active" && (
        ...
        <div className={cardCls}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground">Review and approve advertiser LOI applications.</p>
            </div>
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto">
            {submissions.length === 0 && <p className="text-muted-foreground text-[13px] text-center py-4">No submissions yet.</p>}
            {submissions.map((sub: any) => (
              <div key={sub.id} className="rounded-xl border border-border/30 p-4 space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[12px] text-muted-foreground">{sub.email}</p>
                    </div>
                  </div>
                ...
                {sub.signature_url && (
                  <div>
                    <p className="text-[12px] text-muted-foreground mb-1">Signature:</p>
                  ...
                  <div className="space-y-2 pt-2 border-t border-border/20">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="clay-primary text-primary-foreground rounded-xl px-4 py-2 text-[12px] font-semibold flex items-center gap-1.5 disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        disabled={!!processingId}
                        className="rounded-xl px-4 py-2 text-[12px] font-semibold bg-destructive/10 text-destructive border border-destructive/20 flex items-center gap-1.5 disabled:opacity-50"
                      >
                ...
                {sub.status === "approved" && sub.loi_pdf_url && (
                  <div className="flex items-center gap-3">
                    <a
                      ...
                      className="flex items-center gap-1.5 text-primary text-[12px] font-semibold hover:underline w-fit"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-muted-foreground text-[12px] hover:text-primary transition-colors"
                    >
                ...
                {sub.status === "declined" && sub.admin_notes && (
                  <p className="text-destructive text-[12px]">Notes: {sub.admin_notes}</p>
                )}
              </div>
            ))}
          </div>
