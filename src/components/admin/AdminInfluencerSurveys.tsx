    ...
    <div className="space-y-4">
      <GlassCard className="p-4 space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-2">
          <input className="glass-input rounded-xl px-3 py-2 text-[13px]" placeholder="Title" value={draft.title} onChange={(e) => setDraft((p) => ({ ...p, title: e.target.value }))} />
          <input className="glass-input rounded-xl px-3 py-2 text-[13px]" placeholder="Reward amount" type="number" value={draft.reward_amount} onChange={(e) => setDraft((p) => ({ ...p, reward_amount: Number(e.target.value || 0) }))} />
          <input className="glass-input rounded-xl px-3 py-2 text-[13px] md:col-span-2" placeholder="Description" value={draft.description} onChange={(e) => setDraft((p) => ({ ...p, description: e.target.value }))} />
          <input className="glass-input rounded-xl px-3 py-2 text-[13px]" placeholder="Completion link" value={draft.completion_link} onChange={(e) => setDraft((p) => ({ ...p, completion_link: e.target.value }))} />
          <input className="glass-input rounded-xl px-3 py-2 text-[13px]" placeholder="Completion instructions" value={draft.completion_instructions} onChange={(e) => setDraft((p) => ({ ...p, completion_instructions: e.target.value }))} />
        </div>
        ...
        <div className="space-y-3">
          {draft.questions.map((question, qi) => (
            <div key={qi} className="border rounded-xl p-3 space-y-2">
              <input className="glass-input rounded-xl px-3 py-2 text-[13px] w-full" placeholder={`Question ${qi + 1}`} value={question.question_text} onChange={(e) => setDraft((p) => {
              ...
              {question.options.map((opt, oi) => (
                <div key={`${qi}-${oi}`} className="flex items-center gap-2">
                  <input className="glass-input rounded-xl px-3 py-2 text-[13px] flex-1" placeholder={`Option ${oi + 1}`} value={opt.option_text} onChange={(e) => setDraft((p) => {
                    const questionsDraft = [...p.questions];
              ...
              <GlassButton className="text-[13px]" variant="outline" onClick={() => setDraft((p) => {
                const questionsDraft = [...p.questions];
                questionsDraft[qi] = {
                  ...questionsDraft[qi],
      ...
      <div className="space-y-3">
        {loading ? <p className="text-[13px] text-muted-foreground">Loading influencer surveys...</p> : surveys.map((survey) => {
            ...
            <GlassCard key={survey.id} className="p-4 space-y-2">
              <div className="flex justify-between items-start gap-2">
                <div>
                  ...
                  <p className="text-[13px] text-muted-foreground">{formatNaira(Number(survey.reward_amount))} · {sQuestions.length} questions</p>
                </div>
                <div className="flex gap-2">
              ...
              {sQuestions.map((q) => {
                ...
                return (
                  <div key={q.id} className="border rounded-lg p-2">
                    ...
                    <ul className="mt-1 text-[13px] text-muted-foreground space-y-1">
                      {qOpts.map((opt) => (
