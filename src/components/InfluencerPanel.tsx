      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
          <div className="flex items-center border-b border-border px-3">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search banks..." className="flex-1 bg-transparent py-2.5 px-2 text-sm outline-none placeholder:text-muted-foreground" />
          </div>
          <div className="max-h-60 overflow-y-auto p-1">
            {filtered.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No bank found.</p>}
            {filtered.map(b => (
              <button key={b.code} type="button" onClick={() => { onSelect(b.name); setOpen(false); setSearch(""); }}
                className={cn("w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-colors", selectedBank === b.name ? "bg-primary/10 text-primary" : "hover:bg-muted/50 text-foreground")}>
...
const InfluencerPanel = () => {
    ...
    return (
      <GlassCard variant="glow" className="p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-foreground text-sm">Become an Influencer</h3>
        </div>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Select Platform</label>
            <select
          ...
          {socialPlatform && (
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{socialPlatform.charAt(0).toUpperCase() + socialPlatform.slice(1)} Profile Link</label>
              <input
                value={socialLink}
  ...
  if (application.status === "pending_review") {
    return (
      <GlassCard className="p-5 text-center">
        ...
        <h3 className="font-semibold text-foreground text-sm mb-2">Application Under Review</h3>
        <p className="text-muted-foreground text-sm">We're reviewing your social media profile. You'll be notified when approved.</p>
        <p className="text-xs text-primary mt-3">Submitted: {new Date(application.social_link ? application.social_link : "").toLocaleDateString() || "Recently"}</p>
      </GlassCard>
    );
  }
      ...
      <GlassCard className="p-5 text-center">
        ...
        {!hasAppealed && canAppealNow && (
          <div className="mt-4 space-y-3">
            <p className="text-xs text-primary">You can submit one appeal with an updated profile link.</p>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Select Platform</label>
            ...
            <GlassButton variant="primary" onClick={async () => {
              ...
              setApplying(false);
              await fetchData();
            }} disabled={applying || !socialLink.trim() || !socialPlatform} className="w-full text-sm">
              {applying ? "Submitting..." : "Submit Appeal"}
            </GlassButton>
          </div>
        )}
        {!hasAppealed && !canAppealNow && canAppealAfter && (
          <p className="text-xs text-muted-foreground mt-3">You can appeal after {canAppealAfter.toLocaleString()}</p>
        )}
      </GlassCard>
    );
  }
  ...
  if (application.status === "pending_appeal") {
    return (
      <GlassCard className="p-5 text-center">
        <Clock className="w-10 h-10 text-primary/40 mx-auto mb-3" />
        <h3 className="font-semibold text-foreground text-sm mb-2">Appeal Under Review</h3>
        <p className="text-muted-foreground text-sm">Your appeal is being reviewed. You'll be notified when a decision is made.</p>
      </GlassCard>
    );
  }
  ...
  if (application.status === "approved" && (!wallet || wallet.status === "pending_activation")) {
      ...
      return (
        <GlassCard variant="glow" className="p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-foreground text-sm">Application Approved!</h3>
          </div>
          <p className="text-muted-foreground text-sm mb-4">
          ...
          <div className="space-y-3">
            ...
            {/* ID Upload */}
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Government ID (NIN, Passport, etc.)</label>
              <input ref={idInputRef} type="file" accept="image/*,.pdf" onChange={handleUploadId} className="hidden" />
              <GlassButton variant="outline" onClick={() => idInputRef.current?.click()} disabled={uploading} className="w-full text-sm">
    ...
    return (
      <GlassCard className="p-5 text-center">
        <Clock className="w-10 h-10 text-primary/40 mx-auto mb-3" />
        <h3 className="font-semibold text-foreground text-sm mb-2">Wallet Activation Pending</h3>
        <p className="text-muted-foreground text-sm">Admin is reviewing your bank details and ID. You'll be notified when your wallet is activated.</p>
      </GlassCard>
    );
  }
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <GlassCard variant="glow" className="p-5 flex flex-col h-full">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-foreground text-sm">Influencer Wallet</h3>
            </div>
            <div className="text-center py-2 flex-grow flex flex-col justify-center">
              <p className="text-muted-foreground uppercase tracking-[0.2em] text-xs">Balance</p>
              <h2 className="font-display text-2xl font-bold gradient-text">{formatNaira(wallet.balance)}</h2>
              <p className="text-muted-foreground text-xs mt-1">Total earned: {formatNaira(totalEarned)}</p>
            </div>
            <div className="space-y-3 mt-4">
              ...
              <GlassButton variant="primary" onClick={handleWithdraw} disabled={withdrawing} className="w-full text-sm">
          ...
          <GlassCard variant="glow" className="p-5 flex flex-col h-full">
            <div className="flex items-center gap-2 mb-3">
              <Star className="w-5 h-5 text-primary" />
              <h3 className="font-semibold text-foreground text-sm">Influencer Surveys</h3>
            </div>
        ...
        <GlassCard className="p-5">
          <div className="flex gap-2 mb-6">
            <button onClick={() => setTab("overview")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${tab === "overview" ? "bg-primary/10 text-primary" : "glass-button text-muted-foreground"}`}>
              Overview
            </button>
            <button onClick={() => setTab("withdraw")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${tab === "withdraw" ? "bg-primary/10 text-primary" : "glass-button text-muted-foreground"}`}>
              Withdraw
            </button>
            <button onClick={() => setTab("challenges")} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${tab === "challenges" ? "bg-primary/10 text-primary" : "glass-button text-muted-foreground"}`}>
              Challenges
            </button>
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div className="glass rounded-xl p-3 text-center">
                  <p className="text-sm font-bold text-primary">{referrals.length}</p>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-tight opacity-80">Referrals</p>
                </div>
                <div className="glass rounded-xl p-3 text-center">
                  <p className="text-sm font-bold text-foreground">{formatNaira(totalEarned)}</p>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-tight opacity-80">Earned</p>
                </div>
                <div className="glass rounded-xl p-3 text-center">
                  <p className="text-sm font-bold text-foreground">{withdrawals.filter(w => w.status === "approved").length}</p>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-tight opacity-80">Withdrawals</p>
                </div>
              </div>
              ...
              {influencerLink && (
                <div className="glass rounded-xl p-3">
                  ...
                  <p className="text-xs text-foreground font-mono truncate">{influencerLink}</p>
                  <GlassButton variant="outline" onClick={() => { navigator.clipboard.writeText(influencerLink); toast({ title: "Copied!" }); }} className="w-full mt-2 text-xs">
                    Copy Referral Link
                  </GlassButton>
                </div>
              )}
              ...
              {bankAccount && (
                <div className="glass rounded-xl p-3">
                  ...
                  <p className="text-foreground text-sm font-semibold">{bankAccount.account_name}</p>
                  <p className="text-muted-foreground text-xs">{bankAccount.bank_name} • {bankAccount.account_number}</p>
                </div>
              )}
              ...
              {referrals.length > 0 && (
                <div>
                  <p className="text-sm text-muted-foreground font-medium mb-2">Recent Referrals</p>
                  <div className="space-y-1 max-h-[200px] overflow-y-auto">
                    {referrals.slice(0, 10).map(r => (
                      <div key={r.id} className="flex items-center justify-between glass rounded-lg p-2">
                        <p className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</p>
                        <p className="text-xs text-primary font-semibold">+{formatNaira(r.reward_amount)}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {tab === "withdraw" && (
            <div className="space-y-4">
              ...
              {withdrawals.length > 0 && (
                <div>
                  <p className="text-sm text-muted-foreground font-medium mb-2">Withdrawal History</p>
                  <div className="space-y-1">
                    {withdrawals.map(w => (
                      <div key={w.id} className="flex items-center justify-between glass rounded-lg p-2">
                        <div>
                          <p className="text-xs text-foreground font-semibold">{formatNaira(w.amount)}</p>
                          <p className="text-xs text-muted-foreground font-medium">{new Date(w.created_at).toLocaleDateString()}</p>
                        </div>
                        <span className={`text-xs uppercase font-bold tracking-widest px-2.5 py-0.5 rounded-full ${w.status === "approved" ? "bg-primary/10 text-primary" : w.status === "rejected" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
                          {w.status}
                        </span>
                      </div>
                    ))}
              ...
              <div className="flex gap-1 bg-muted/30 rounded-lg p-1">
                {(["new", "ongoing", "past"] as const).map(st => (
                  <button key={st} onClick={() => setChallengeSubTab(st)}
                    className={`flex-1 py-1.5 rounded-md text-xs font-medium transition-all capitalize ${challengeSubTab === st ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}>
                ...
                if (currentList.length === 0) {
                  return (
                    <div className="text-center py-6">
                      ...
                      <p className="text-muted-foreground text-sm">{emptyMessages[challengeSubTab]}</p>
                    </div>
                  );
                }
                    ...
                    <div key={ch.id} className="glass rounded-xl p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs text-primary font-semibold flex items-center gap-1"><Hash className="w-3 h-3" />{ch.hashtag}</span>
                            <span className="text-xs text-muted-foreground">{formatNaira(ch.reward_per_video)}/video</span>
                          </div>
                        </div>
                        {isComplete && <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">Completed ✓</span>}
                      </div>
                      ...
                      {ch.instructions && (
                        <div className="glass rounded-lg p-3 mb-3">
                          ...
                          <p className="text-sm text-foreground whitespace-pre-wrap">{ch.instructions}</p>
                        </div>
                      )}
                      ...
                      {enrollment && !isComplete && canSubmitNext && (
                        <div className="space-y-2">
                          {!canPostNow && nextPostDate && (
                            <p className="text-xs text-primary text-center">Next video can be submitted after {nextPostDate.toLocaleString()}</p>
                          )}
                          {canPostNow && (
                            <>
                              <div>
                                <label className="text-xs text-muted-foreground mb-1 block">Video #{nextVideoNumber} Link (social media post URL)</label>
                                <input
                              ...
                              <GlassButton variant="primary" onClick={async () => {
                                ...
                                await fetchData();
                              }} disabled={!videoLinks[ch.id]?.trim()} className="w-full text-sm">
                      ...
                      {mySubs.length > 0 && (
                        <div className="mt-3">
                          <p className="text-xs text-muted-foreground font-semibold mb-1">Your Submissions</p>
                          <div className="space-y-1 max-h-[150px] overflow-y-auto">
                            {mySubs.map(s => (
                              <div key={s.id} className="flex items-center justify-between glass rounded-lg p-2">
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-muted-foreground">#{s.video_number}</span>
                                  <a href={s.video_url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline truncate max-w-[120px]">{s.video_url}</a>
                                </div>
                                <span className={`text-xs uppercase font-bold tracking-widest px-2.5 py-0.5 rounded-full ${s.status === "approved" ? "bg-primary/10 text-primary" : s.status === "rejected" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
