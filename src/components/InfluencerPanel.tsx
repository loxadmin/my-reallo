      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            {filtered.length === 0 && <p className="text-[13px] text-muted-foreground text-center py-4">No bank found.</p>}
            {filtered.map(b => (
  ...
  if (application.status === "pending_appeal") {
    return (
      <GlassCard className="p-5 text-center">
        ...
        <p className="text-muted-foreground text-[13px]">Your appeal is being reviewed. You'll be notified when a decision is made.</p>
      </GlassCard>
    );
  }
  ...
  if (application.status === "approved" && (!wallet || wallet.status === "pending_activation")) {
    ...
    return (
      <GlassCard className="p-5 text-center">
        ...
        <p className="text-muted-foreground text-[13px]">Admin is reviewing your bank details and ID. You'll be notified when your wallet is activated.</p>
      </GlassCard>
    );
  }
                ...
                if (currentList.length === 0) {
                  return (
                    <div className="text-center py-6">
                      ...
                      <p className="text-muted-foreground text-[13px]">{emptyMessages[challengeSubTab]}</p>
                    </div>
                  );
                }
                          ...
                          {canPostNow && (
                            <>
                              ...
                              <GlassButton variant="primary" onClick={async () => {
                                ...
                                await fetchData();
                              }} disabled={!videoLinks[ch.id]?.trim()} className="w-full text-[13px]">
