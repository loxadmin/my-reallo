    ...
    return (
      <GlassCard variant="glow" className="p-5">
        ...
        <div className="space-y-3">
          <div>
            <label className="text-sm text-muted-foreground mb-1 block">Select Platform</label>
            <select
          ...
          {socialPlatform && (
            <div>
              <label className="text-sm text-muted-foreground mb-1 block">{socialPlatform.charAt(0).toUpperCase() + socialPlatform.slice(1)} Profile Link</label>
              <input
                value={socialLink}
      ...
      <GlassCard className="p-5 text-center">
        ...
        {!hasAppealed && canAppealNow && (
          <div className="mt-4 space-y-3">
            <p className="text-sm text-primary">You can submit one appeal with an updated profile link.</p>
            <div>
              <label className="text-sm text-muted-foreground mb-1 block">Select Platform</label>
      ...
      return (
        <GlassCard variant="glow" className="p-5">
          ...
          <div className="space-y-3">
            ...
            {/* ID Upload */}
            <div>
              <label className="text-sm text-muted-foreground mb-1 block">Government ID (NIN, Passport, etc.)</label>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <GlassCard variant="glow" className="p-5 flex flex-col h-full">
            ...
            <div className="text-center py-2 flex-grow flex flex-col justify-center">
              <p className="text-muted-foreground uppercase tracking-[0.14em] text-xs">Balance</p>
              <h2 className="font-display text-2xl font-bold gradient-text">{formatNaira(wallet.balance)}</h2>
              <p className="text-muted-foreground text-sm mt-1">Total earned: {formatNaira(totalEarned)}</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              {influencerLink && (
                <div className="glass rounded-xl p-3">
                  ...
                  <GlassButton variant="outline" onClick={() => { navigator.clipboard.writeText(influencerLink); toast({ title: "Copied!" }); }} className="w-full mt-2 text-sm">
                    Copy Referral Link
                  </GlassButton>
                </div>
              )}
              ...
              {bankAccount && (
                <div className="glass rounded-xl p-3">
                  ...
                  <p className="text-muted-foreground text-sm">{bankAccount.bank_name} • {bankAccount.account_number}</p>
                </div>
              )}
                  ...
                  <div className="space-y-1 max-h-[200px] overflow-y-auto">
                    {referrals.slice(0, 10).map(r => (
                      <div key={r.id} className="flex items-center justify-between glass rounded-lg p-2">
                        ...
                        <p className="text-sm text-primary font-semibold">+{formatNaira(r.reward_amount)}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    {withdrawals.map(w => (
                      <div key={w.id} className="flex items-center justify-between glass rounded-lg p-2">
                        <div>
                          <p className="text-sm text-foreground font-semibold">{formatNaira(w.amount)}</p>
                          <p className="text-sm text-muted-foreground">{new Date(w.created_at).toLocaleDateString()}</p>
                        </div>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${w.status === "approved" ? "bg-primary/10 text-primary" : w.status === "rejected" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
                          {w.status}
                        </span>
                      </div>
                    ))}
              ...
              <div className="flex gap-1 bg-muted/30 rounded-lg p-1">
                {(["new", "ongoing", "past"] as const).map(st => (
                  <button key={st} onClick={() => setChallengeSubTab(st)}
                    className={`flex-1 py-1.5 rounded-md text-sm font-medium transition-all capitalize ${challengeSubTab === st ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}>
                          ...
                          <div className="space-y-1 max-h-[150px] overflow-y-auto">
                            {mySubs.map(s => (
                              <div key={s.id} className="flex items-center justify-between glass rounded-lg p-2">
                                ...
                                <span className={`text-sm px-2 py-0.5 rounded-full ${s.status === "approved" ? "bg-primary/10 text-primary" : s.status === "rejected" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
