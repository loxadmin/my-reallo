  ...
  const renderTasks = () => {
        ...
        return (
          <GlassCard variant="glow" className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-muted-foreground">{pendingInteractions.length + 1} remaining</p>
            </div>
          ...
          return (
            <GlassCard variant="glow" className="space-y-4">
              ...
              {switchApp.referral_message && (
                <p className="text-xs text-muted-foreground">{switchApp.referral_message}</p>
              )}
            ...
            <motion.div key={earnTab} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-2">
              {currentList.length === 0 && (
                <GlassCard className="text-center py-8">
                  <p className="text-muted-foreground text-sm">No {earnTab === "earn" ? "available" : earnTab} earnings</p>
                </GlassCard>
              )}
                ...
                return (
                  <GlassCard key={resp.id} className="p-4" animate={false}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        {resp.points_awarded > 0 && (
                          <p className="text-sm text-primary font-semibold">+{resp.points_awarded} pts</p>
                        )}
                      </div>
                    </div>
                    ...
                    {(app.category === "yes_no" || app.category === "robust") && resp.would_switch === true && !resp.switch_completed && (
                      <div className="mt-3">
                        {canSwitch ? (
                          <GlassButton variant="primary" onClick={() => handleSwitchComplete(app)} className="w-full text-sm">
                    ...
                    {(app.category === "yes_no" || app.category === "robust") && resp.switch_completed && (
                      <p className="text-xs text-primary mt-2">✓ Switched</p>
                    )}
                    ...
                    {app.category === "referral" && resp.referral_screenshot_url && !resp.referral_approved && (
                      <p className="text-xs text-muted-foreground mt-2">📋 Screenshot pending admin review</p>
                    )}

                    {app.category === "referral" && resp.referral_approved && (
                      <p className="text-xs text-primary mt-2">✓ Approved — {app.referral_points} pts awarded</p>
                    )}
                  </GlassCard>
                );
              })}
    ...
    if (unansweredApps.length === 0) return (
      <GlassCard className="text-center py-8">
        ...
        <p className="text-muted-foreground text-sm">No new apps to review. Check back later!</p>
      </GlassCard>
    );
          ...
          {unansweredApps.map((app) => (
            ...
            >
              ...
              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${
                ...
                {selectedApps.has(app.id) && <span className="text-primary-foreground text-xs font-bold leading-none">✓</span>}
              </div>
            </button>
          ))}
        </div>
  ...
  const renderSurveys = () => {
              ...
              {surveyStep === "quiz" && question && (
                <motion.div key={question.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs uppercase tracking-widest bg-primary/10 text-primary px-2.5 py-1 rounded-full font-bold">
                  ...
                  <div className="p-4 rounded-xl bg-muted/20 border border-border/40 text-left space-y-3">
                    ...
                    {activeSurvey.completion_link && (
                      <a href={activeSurvey.completion_link} target="_blank" rel="noopener noreferrer">
                        <GlassButton variant="primary" className="w-full text-sm">
    ...
    return (
      <div className="space-y-3">
        {surveys.length === 0 && (
          <GlassCard className="text-center py-8">
            <p className="text-muted-foreground text-sm">No active surveys available.</p>
          </GlassCard>
        )}
          ...
          return (
            <GlassCard key={survey.id} className="p-4 overflow-hidden relative">
              <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  <p className="text-sm text-primary font-bold">+{survey.points_reward} pts</p>
                </div>
              </div>

              {!response && (
                <div className="mt-3">
                  <GlassButton variant="primary" onClick={() => handleStartSurvey(survey)} className="w-full text-sm">
              ...
              {isInProgress && (
                <div className="mt-3 space-y-2">
                  <div className="flex items-center gap-2 text-xs text-accent-foreground bg-accent/10 p-2 rounded-lg border border-accent/20">
                    <Clock className="w-3.5 h-3.5" />
                    <span>Quiz passed. {timeLeft} to upload proof.</span>
                  </div>
                  <GlassButton variant="outline" onClick={() => handleStartSurvey(survey)} className="w-full text-sm">
                    Continue Survey
                  </GlassButton>
                </div>
              )}
              ...
              {isPending && (
                <div className="mt-3 flex items-center gap-2 text-xs text-accent-foreground bg-accent/10 p-2 rounded-lg border border-accent/20">
              ...
              {isApproved && (
                <div className="mt-3 flex items-center gap-2 text-xs text-primary bg-primary/10 p-2 rounded-lg border border-primary/20">
