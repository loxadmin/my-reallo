...
import { ArrowRight, Zap, Wifi, UtensilsCrossed, Bus } from "lucide-react";
...
const SpendCalculator = ({ onComplete }: SpendCalculatorProps) => {
  ...
  const handleComplete = () => {
    onComplete({
      weeklyData: Number(weeklyData), monthlyElectricity: Number(monthlyElectricity),
      weeklyFood: Number(weeklyFood), weeklyTransport: Number(weeklyTransport),
      annualData, annualElectricity, annualFood, annualTransport, totalAnnual,
    });
  };
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.4 }}>
              <GlassCard variant="glow">
                ...
                <GlassInput label="Weekly food spend" prefix="₦" type="number" placeholder="e.g. 10000" value={weeklyFood} onChange={(e) => setWeeklyFood(e.target.value)} min="0" />
                {weeklyFood && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[12px] text-primary mt-3">
                    Annual: {formatNaira(Number(weeklyFood) * 52)}
                  </motion.p>
                )}
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.4 }}>
              <GlassCard variant="glow">
                ...
                <GlassInput label="Weekly transport spend" prefix="₦" type="number" placeholder="e.g. 5000" value={weeklyTransport} onChange={(e) => setWeeklyTransport(e.target.value)} min="0" />
                {weeklyTransport && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[12px] text-primary mt-3">
                    Annual: {formatNaira(Number(weeklyTransport) * 52)}
                  </motion.p>
                )}
