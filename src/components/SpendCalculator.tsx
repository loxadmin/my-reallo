...
const SpendCalculator = ({ onComplete }: SpendCalculatorProps) => {
  ...
  const handleComplete = () => {
    onComplete({ weeklyData: Number(weeklyData), monthlyElectricity: Number(monthlyElectricity), annualData, annualElectricity, totalAnnual });
  };
          ...
          {step === 0 && (
            <motion.div key="step0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.4 }}>
              <GlassCard variant="glow">
                ...
                {weeklyData && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-primary/70 mt-3 font-display">
                    Annual: {formatNaira(Number(weeklyData) * 52)}
                  </motion.p>
                )}
                <GlassButton variant="primary" className="w-full mt-6" onClick={handleNext} disabled={!weeklyData}>
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.4 }}>
              <GlassCard variant="glow">
                ...
                {monthlyElectricity && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-accent/70 mt-3 font-display">
                    Annual: {formatNaira(Number(monthlyElectricity) * 12)}
                  </motion.p>
                )}
                <GlassButton variant="primary" className="w-full mt-6" onClick={handleNext} disabled={!monthlyElectricity}>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
              <GlassCard variant="glow" className="text-center">
                <motion.p className="text-sm text-muted-foreground font-display uppercase tracking-widest mb-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
                  Your Annual Spend
                </motion.p>
                <motion.h2 className="font-display text-4xl sm:text-5xl font-bold gradient-text mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
