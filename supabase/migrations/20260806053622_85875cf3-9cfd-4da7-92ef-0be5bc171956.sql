...
BEGIN
  ...
  SELECT * INTO v_me FROM public.monthly_earners WHERE user_id = v_user_id AND status = 'active';

  IF v_me.id IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM public.influencer_applications
                     WHERE user_id = v_user_id AND status = 'approved') THEN
    SELECT COUNT(*)::int INTO v_valid_count FROM public.influencer_referrals
      WHERE influencer_id = v_user_id AND status = 'valid'
        AND validated_at >= v_me.cycle_start;
    IF v_valid_count < GREATEST(40, v_me.target_referrals) THEN
      RETURN json_build_object('error',
        format('Monthly Earners need %s valid referrals this cycle to withdraw. Current: %s',
               GREATEST(40, v_me.target_referrals), v_valid_count));
    END IF;
  ELSE
